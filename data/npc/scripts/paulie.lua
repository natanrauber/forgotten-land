...
local function greetCallback(cid)
	...
	if player:getStorageValue(Storage.TheRookieGuard.Mission08) == 1 then
		npcHandler:setMessage(
			MESSAGE_GREET,
			"Welcome |PLAYERNAME|! Special newcomer offer, today only! {Deposit} some money - or {deposit ALL} of your money! - and get 50 gold for free!"
		)
	else
		npcHandler:setMessage(
			MESSAGE_GREET,
			"Welcome |PLAYERNAME|! Here, you can {deposit} or {withdraw} your money from your bank account and {change} gold. I can also explain the {functions} of your bank account to you."
		)
	end
	return true
end
...
keywordHandler:addKeyword(
	...
	function(cid, message, keywords, parameters, node)
		...
		elseif player:getBankBalance() >= 10000000 then
			npcHandler:say(
				"You have made ten millions and it still grows! Your account balance is " .. player:getBankBalance() .. " gold.",
				cid
			)
		...
		elseif player:getBankBalance() >= 100000 then
			npcHandler:say(
				"You certainly have made a pretty penny. Your account balance is " .. player:getBankBalance() .. " gold.",
				cid
			)
		else
...
local function depositHandler(cid, message, keywords, parameters, node)
	...
	elseif player:getBankBalance() + npcHandler.topic[cid] <= 1000 then
		...
		if player:getBankBalance() < 1000 then
			...
			npcHandler:say(
				"Uh oh! I fear all that gold would exceed the limit of your Rookgaard bank account which are 1000 gold pieces. I can deposit " ..
					npcHandler.topic[cid] .. " gold though. Would you like that?",
				cid
			)
			return true
		else
			npcHandler:say(
				"Uh oh! I fear all that gold would exceed the limit of your Rookgaard bank account which are 1000 gold pieces.",
				cid
			)
			return false
		end
		...
		if depositBonus and player:depositMoney(npcHandler.topic[cid]) then
			...
			npcHandler:say(
				"Alright, we have added the amount of " ..
					(npcHandler.topic[cid] + depositBonus) ..
						" gold to your {balance} - that is the money you deposited plus a bonus of " ..
							depositBonus .. " gold. Thank you! You can {withdraw} your money anytime.",
				cid
			)
		elseif player:depositMoney(npcHandler.topic[cid]) then
			npcHandler:say(
				"Alright, we have added the amount of " ..
					npcHandler.topic[cid] .. " gold to your balance. You can withdraw your money anytime.",
				cid
			)
		else
			npcHandler:say("You don't have enough gold.", cid)
...
withdrawAmount:addChildKeyword(
	{"yes"},
	function(cid, message, keywords, parameters, node)
		local npcHandler = parameters.npcHandler
		if npcHandler == nil then
		...
		if player:getFreeCapacity() >= getMoneyWeight(npcHandler.topic[cid]) then
			if player:withdrawMoney(npcHandler.topic[cid]) then
				npcHandler:say(
					"Here you are, " ..
						npcHandler.topic[cid] .. " gold. Please let me know if there is something else I can do for you.",
					cid
				)
			else
				npcHandler:say("There is not enough gold on your account.", cid)
			...
			npcHandler:say(
				"Whoah, hold on, this is too heavy for you. I don't want you to drop it on the floor, maybe come back with a cart!",
				cid
			)
		end
		npcHandler:resetNpc(cid)
...
local changeGoldAmount =
	changeGold:addChildKeyword(
	{"%d+"},
	function(cid, message, keywords, parameters, node)
		local npcHandler = parameters.npcHandler
		if npcHandler == nil then
		...
		if npcHandler.topic[cid] == 0 then
			...
			npcHandler:say(
				"So you want me to change " ..
					(npcHandler.topic[cid] * 100) .. " of your gold coins into " .. npcHandler.topic[cid] .. " platinum coins?",
				cid
			)
			return true
		end
...
changeGoldAmount:addChildKeyword(
	{"yes"},
	function(cid, message, keywords, parameters, node)
		local npcHandler = parameters.npcHandler
		if npcHandler == nil then
...
local changePlatinumToGold =
	changePlatinum:addChildKeyword(
	{"gold"},
	StdModule.say,
	{
		npcHandler = npcHandler,
...
local changePlatinumToGoldAmount =
	changePlatinumToGold:addChildKeyword(
	{"%d+"},
	function(cid, message, keywords, parameters, node)
		local npcHandler = parameters.npcHandler
		if npcHandler == nil then
		...
		if npcHandler.topic[cid] <= 0 then
			...
			npcHandler:say(
				"So you want me to change " ..
					npcHandler.topic[cid] .. " of your platinum coins into " .. (npcHandler.topic[cid] * 100) .. " gold coins for you?",
				cid
			)
			return true
		end
...
changePlatinumToGoldAmount:addChildKeyword(
	{"yes"},
	function(cid, message, keywords, parameters, node)
		local npcHandler = parameters.npcHandler
		if npcHandler == nil then
...
local changePlatinumToCrystalAmount =
	changePlatinumToCrystal:addChildKeyword(
	{"%d+"},
	function(cid, message, keywords, parameters, node)
		local npcHandler = parameters.npcHandler
		if npcHandler == nil then
			...
			npcHandler:say(
				"So you want me to change " ..
					(npcHandler.topic[cid] * 100) ..
						" of your platinum coins into " .. npcHandler.topic[cid] .. " crystal coins for you?",
				cid
			)
			return true
		end
...
changePlatinumToCrystalAmount:addChildKeyword(
	{"yes"},
	function(cid, message, keywords, parameters, node)
		local npcHandler = parameters.npcHandler
		if npcHandler == nil then
...
local changeCrystalAmount =
	changeCrystal:addChildKeyword(
	{"%d+"},
	function(cid, message, keywords, parameters, node)
		local npcHandler = parameters.npcHandler
		if npcHandler == nil then
		...
		if npcHandler.topic[cid] == 0 then
			...
			npcHandler:say(
				"So you want me to change " ..
					npcHandler.topic[cid] .. " of your crystal coins into " .. (npcHandler.topic[cid] * 100) .. " platinum coins?",
				cid
			)
			return true
		end
...
changeCrystalAmount:addChildKeyword(
	{"yes"},
	function(cid, message, keywords, parameters, node)
		local npcHandler = parameters.npcHandler
		if npcHandler == nil then
...
keywordHandler:addKeyword(
	...
	{
		npcHandler = npcHandler,
		text = "I'm still a bank clerk-in-training. They say I can work on the {mainland} of Tibia once I have proven myself!"
	}
)
keywordHandler:addKeyword(
	{"mainland"},
	...
	{
		npcHandler = npcHandler,
		text = "I'm looking forward to work on the mainland. The great Tibian cities are so much more interesting than this little village of {Rookgaard}."
	}
)
keywordHandler:addKeyword(
	{"rookgaard"},
	...
	{
		npcHandler = npcHandler,
		text = "What a godforsaken place! Well, at least there are no criminals robbing the bank which would be - me. <gulp>"
	}
)
keywordHandler:addKeyword(
	{"trade"},
	...
	{
		npcHandler = npcHandler,
		text = "Sorry, I don't trade. Ask the shop owners for a trade instead. I can only help you with your bank account."
	}
)
...
keywordHandler:addKeyword(
	...
	{
		npcHandler = npcHandler,
		text = "Every Tibian has a bank account. You can {deposit} your gold in one bank and {withdraw} it from the same or any other Tibian bank. Ask me for your {balance} to learn how much money you've already saved. There are also {advanced} functions."
	}
)
keywordHandler:addKeyword(
	{"quest"},
	...
	{
		npcHandler = npcHandler,
		text = "Well, the greatest quest in life must be to amass tons of gold coins! Good luck with that!"
	}
)
...
keywordHandler:addKeyword(
	...
	{
		npcHandler = npcHandler,
		text = "You can {deposit} and {withdraw} money from your {bank account} here. I can also {change} money for you."
	}
)
...
keywordHandler:addKeyword(
	...
	{
		npcHandler = npcHandler,
		text = "There are three different coin types in Tibia: 100 gold coins equal 1 platinum coin, 100 platinum coins equal 1 crystal coin. For example, if you like to change 100 gold coins into 1 platinum coin, simply say '{change gold}' and then '1 platinum'."
	}
)
keywordHandler:addKeyword(
	{"advanced"},
	StdModule.say,
	{
		npcHandler = npcHandler,
		text = {
			"Once you are on the Tibian mainland, you can access new functions of your bank account, such as transferring money to other players safely, accessing your guild account or taking part in house auctions. ...",
			"Just ask any of my colleagues on the Mainland and they'll be happy to tell you all about the advanced functions of your bank account."
		}
	}
)
keywordHandler:addKeyword(
	{"transfer"},
	StdModule.say,
	{npcHandler = npcHandler, text = "I'm afraid this service is not available to you until you reach mainland."}
)

npcHandler:setMessage(MESSAGE_WALKAWAY, "Good bye.")
npcHandler:setMessage(
	MESSAGE_FAREWELL,
	"Good bye, and remember: entrusting us with your gold is the safest way of storing it!"
)
